...
const projects = [
  { domain: "afuchat.com",       name: "AfuChat",     desc: "Unified communication platform for the modern web",                      icon: MessageSquare, brand: { primary: "#00BCD4", iconBg: "#e0f7fa", card: "#faf8f3", cardBorder: "#b2ebf2" }, logoUrl: "/__mockup/favicons/afuchat.png" },
  { domain: "email.afuchat.com", name: "AfuMail",     desc: "Smart, privacy-first email for the ecosystem",                           icon: Mail,          brand: { primary: "#3b82f6", iconBg: "#dbeafe", card: "#ffffff", cardBorder: "#bfdbfe" }, logoUrl: "/__mockup/favicons/email.afuchat.com.svg" },
  { domain: "cloud.afuchat.com", name: "AfuCloud",    desc: "Personal cloud storage and file management",                             icon: Cloud,         brand: { primary: "#f97316", iconBg: "#ffedd5", card: "#ffffff", cardBorder: "#fed7aa" }, logoUrl: "/__mockup/favicons/cloud.afuchat.com.ico" },
  { domain: "blog.afuchat.com",  name: "AfuBlog",     desc: "Publish ideas, stories, and long-form content",                          icon: BookOpen,      brand: { primary: "#14b8a6", iconBg: "#ccfbf1", card: "#ffffff", cardBorder: "#99f6e4" }, logoUrl: "/__mockup/favicons/blog.afuchat.com.ico" },
  { domain: "math.afuchat.com",  name: "AfuMath",     desc: "Interactive math education and problem solving",                         icon: GraduationCap, brand: { primary: "#3b82f6", iconBg: "#ede9fe", card: "#ffffff", cardBorder: "#ddd6fe" }, logoUrl: "/__mockup/favicons/math.afuchat.com.png" },
  { domain: "desk.afuchat.com",  name: "AfuDesk",     desc: "Customer support and helpdesk for the ecosystem",                        icon: Headphones,    brand: { primary: "#f97316", iconBg: "#fff7ed", card: "#ffffff", cardBorder: "#fed7aa" }, logoUrl: "/__mockup/favicons/desk.afuchat.com.ico" },
  { domain: "dev.afuchat.com",   name: "AfuChat.dev", desc: "Professional full-stack web and mobile development in Uganda",           icon: Code2,         brand: { primary: "#a855f7", iconBg: "#f3e8ff", card: "#ffffff", cardBorder: "#e9d5ff" }, logoUrl: "/__mockup/favicons/dev.afuchat.com.svg", ctaLabel: "Get a Quote", ctaHref: "https://dev.afuchat.com/estimate" },
];
...
const clients = [
  ...
  { domain: "honeybeeministriesug.org", name: "Honeybee Ministries", desc: "Faith-based organization serving communities across Uganda",                       icon: Globe,      brand: { primary: "#d97706", iconBg: "#fef3c7", card: "#fffbeb", cardBorder: "#fde68a" }, logoUrl: "/__mockup/favicons/honeybeeministriesug.org.ico", founderImg: "https://github.com/afuchat1/honeybee/blob/main/src/assets/founder-portrait.jpg?raw=true", founderLabel: "Founder" },
  { domain: "sabulashoespot.com",        name: "Sabula Shoe Spot",   desc: "Quality footwear retail brand for everyday style",                                  icon: Globe,      brand: { primary: "#f59e0b", iconBg: "#fef9c3", card: "#ffffff", cardBorder: "#fde68a" }, logoUrl: "/__mockup/favicons/sabulashoespot.com.jpg" },
  { domain: "mmradioug.org",             name: "MM Radio Uganda",    desc: "Online radio station keeping Uganda connected through music, news and culture",      icon: Globe,      brand: { primary: "#ef4444", iconBg: "#fef2f2", card: "#ffffff", cardBorder: "#fecaca" }, logoUrl: "/__mockup/favicons/mmradioug.org.png" },
];

const partners = [
  { domain: "ajsdigitalservices.com", name: "AJS Digital Services", desc: "IT training and digital services empowering careers across Africa", icon: Globe, brand: { primary: "#f97316", iconBg: "#fff7ed", card: "#ffffff", cardBorder: "#fed7aa" }, logoUrl: "/__mockup/favicons/ajsdigitalservices.com.png", founderImg: "https://dev.afuchat.com/assets/cofounder-photo-Bw4GhOPz.jpg", founderLabel: "Founder" },
];

const socialLinks = [
  { href: "https://afuchat.com/@amkaweesi", label: "AfuChat",      icon: MessageSquare, logoUrl: "/__mockup/favicons/afuchat.png" },
...
function HeroHeatmap() {
  ...
  // Static baseline data keeps the extracted page deterministic in the sandbox.
  const makeDays = (selectedYear: number): ContribDay[] => {
    ...
    for (let date = new Date(start); date <= end; date.setDate(date.getDate() + 1)) {
      const seed = (date.getDate() * 7 + date.getMonth() * 13 + selectedYear) % 17;
      const count = seed > 10 ? seed - 8 : seed > 5 ? seed - 5 : 0;
      result.push({ date: date.toISOString().slice(0, 10), count, level: count === 0 ? 0 : Math.min(4, Math.ceil(count / 2)) });
    }
    return result;
  };
  const [days, setDays] = useState<ContribDay[]>(() => makeDays(AVAILABLE_YEARS[0]));
  const [total, setTotal] = useState<number>(() => makeDays(AVAILABLE_YEARS[0]).reduce((sum, day) => sum + day.count, 0));

  useEffect(() => {
    const contribs = makeDays(year);
    setDays(contribs);
    setTotal(contribs.reduce((sum, day) => sum + day.count, 0));
  }, [year]);
...
export default function Home() {
  ...
  const trackVisit = (domain: string) => {
    setVisitCounts((prev) => ({ ...prev, [domain]: (prev[domain] ?? 0) + 1 }));
  };
